...
import com.swarm.entities.CubeEntity;
import com.swarm.entities.Entity;
import com.swarm.entities.PyramidEntity;
import com.swarm.entities.SphereEntity;
import com.swarm.entities.WorldAxisEntity;
	...
	public World(List<ObjectList> objectList) {
		...
		for (ObjectList eachObject : getObjectList()) {
			...
			for (ix = 0; ix < eachObject.getQuantity(); ix++) {
				...
				switch (switchTest) {
				case 1:
					CubeEntity cube = new CubeEntity(rnd.nextGaussian()*2, new WorldPoint((double)rnd.nextGaussian() * 100,(double)rnd.nextGaussian() * 100,(double)rnd.nextGaussian() * 100));
					cube.setRgbaColour(rgbaArray);
					cube.translateVector(new WorldPoint((double)rnd.nextGaussian() * 100,(double)rnd.nextGaussian() * 100,(double)rnd.nextGaussian() * 100));
					entities.add(cube);
					break;
				case 2:
					PyramidEntity pyramid = new PyramidEntity(rnd.nextGaussian()*2);
					pyramid.translateVector(new WorldPoint((double)rnd.nextGaussian() * 100,(double)rnd.nextGaussian() * 100,(double)rnd.nextGaussian() * 100));
				...
				case 3:
					...
					SphereEntity sphere = new SphereEntity(0.3);
